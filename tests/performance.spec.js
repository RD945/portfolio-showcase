...
for (const profile of profiles) {
  test(`cold-load performance: ${profile.name}`, async ({ browser }) => {
        ...
        await page.evaluate(() => ({
          ...
          transferredKB: Math.round(
            performance
              ...
              .reduce((sum, resource) => sum + resource.transferSize, 0) / 1024,
          ),
          external: performance
            .getEntriesByType("resource")
            .filter((resource) => !resource.name.startsWith(location.origin)).length,
        })),
      );
    ...
    for (const result of results) {
      expect(result.cls, `${profile.name} CLS budget`).toBeLessThan(0.1);
      expect(result.external, `${profile.name} external resources`).toBe(0);
